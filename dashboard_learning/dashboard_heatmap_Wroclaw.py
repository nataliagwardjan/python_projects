...
app = dash.Dash()
app.layout = html.Div(children=[html.H1('Pogoda we Wrocławiu',
...
def update_heatmap(x_range):
    ...
    fig.append_trace(go.Heatmap(x=wro_temp.index.hour,
                                y=wro_temp.index.weekday.map(
                                    ...
                                    5:'Sobota',
                                    6:'Niedziela'}),
                                z = wro_temp['Wroclaw Temperature [2 m elevation corrected]'].tolist(),
    ...
    fig.append_trace(go.Heatmap(x=wro_temp.index.hour,
                                y=wro_temp.index.weekday.map({
                                    ...
                                    5:'Sobota',
                                    6:'Niedziela'}),
                                z = wro_temp['Wroclaw Precipitation Total'].tolist(),
                                colorscale='Cividis',
                                showscale=False), 1, 2)
    return fig
